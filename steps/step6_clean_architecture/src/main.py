...
インフラ層のリポジトリ具象実装、各ユースケース、CLI のインスタンス化と
依存性の注入 (Composition Root) を行い、
アプリケーションを起動します。

クリーンアーキテクチャに従い、最外層であるエントリーポイントで
全ての依存性を解決して注入します。
"""

from infrastructure.repository.cart_repository import InMemoryCartRepository
from infrastructure.repository.order_repository import InMemoryOrderRepository
...
def main() -> None:
    ...
    # 3. CLI の初期化 (プレゼンテーション層にユースケース群を注入)
    cli = CLI(
